...
const labels = {
  de: {
    ...
    noMessages: 'Noch keine Mitteilungen vorhanden.', unread: 'Ungelesen', markRead: 'Als gelesen markieren', role: 'Rolle', language: 'Sprache', push: 'Push-Benachrichtigungen', pushIntro: 'Erhalten Sie wichtige BvL-Mitteilungen direkt auf diesem Gerät.', pushEnable: 'Push aktivieren', pushDisable: 'Push auf diesem Gerät deaktivieren', pushMissing: 'Push ist noch nicht eingerichtet. In Coolify müssen VAPID-Schlüssel hinterlegt werden.', pushIos: 'Auf dem iPhone funktioniert Push erst nach „Zum Home-Bildschirm“ und dem Öffnen der installierten App.',
    changePassword: 'Passwort ändern', currentPassword: 'Aktuelles Passwort', newPassword: 'Neues Passwort (mindestens 12 Zeichen)', savePassword: 'Passwort speichern', signOut: 'Abmelden', openAdmin: 'Administration öffnen',
  ...
  en: {
    ...
    noMessages: 'No messages yet.', unread: 'Unread', markRead: 'Mark as read', role: 'Role', language: 'Language', push: 'Push notifications', pushIntro: 'Receive important BvL updates directly on this device.', pushEnable: 'Enable push', pushDisable: 'Disable push on this device', pushMissing: 'Push is not configured yet. VAPID keys must be added in Coolify.', pushIos: 'On iPhone, push requires adding the app to the Home Screen and opening the installed app.',
    changePassword: 'Change password', currentPassword: 'Current password', newPassword: 'New password (at least 12 characters)', savePassword: 'Save password', signOut: 'Sign out', openAdmin: 'Open administration',
...
function renderProfile() {
  ...
  return `<main class="main-content content-narrow"><h1 class="page-title">${esc(t('profile'))}</h1><p class="page-intro">${esc(user.displayName)} · ${esc(user.role)}</p>
    <div class="profile-grid">
      ...
      <section class="profile-panel"><h2>${esc(t('changePassword'))}</h2><form data-form="password" class="form-grid"><label class="field">${esc(t('currentPassword'))}<input name="currentPassword" type="password" autocomplete="current-password" required></label><label class="field">${esc(t('newPassword'))}<input name="newPassword" type="password" minlength="12" autocomplete="new-password" required></label><div class="span-2"><button class="button secondary" type="submit">${esc(t('savePassword'))}</button></div></form></section>
...
function renderUserEditor() {
  ...
  return `<aside class="editor-panel"><h2>${esc(isNew ? t('newUser') : item.displayName)}</h2><form data-form="user">
    ...
    <label class="field">${esc(isNew ? t('createPassword') : t('resetPassword'))}<input name="password" type="password" ${isNew ? 'required minlength="12"' : ''} autocomplete="new-password"></label>
